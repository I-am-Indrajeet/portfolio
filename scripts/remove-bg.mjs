...
const input = join(__dirname, '../public/hero_section.png')
const output = join(__dirname, '../public/hero_section_no_bg.png')
...
function addBackgroundPixel(index) {
  ...
  const pixel = index * 4
  if (!isBackground(pixels[pixel], pixels[pixel + 1], pixels[pixel + 2])) return
...
for (let index = 0; index < background.length; index++) {
  ...
  if (backgroundNeighbors >= 4) pixels[pixel + 3] = Math.min(pixels[pixel + 3], 120)
  else if (backgroundNeighbors >= 2) pixels[pixel + 3] = Math.min(pixels[pixel + 3], 220)
